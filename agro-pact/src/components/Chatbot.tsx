...
// Chat data (equivalent to what is in index.js)
interface ChatStep {
    question?: string;
...
const chatData: ChatData = {
    ...
    flow: {
        "Contract Related Issue": [
            ...
            { question: "Thank you, NAME. Could you share your phone number?", field: "phone" },
            { question: "Can you provide the Contract ID?", field: "contractId" },
            { response: "Thank you! We've logged your inquiry for contract CONTRACTID." }
        ],
        "Billing Inquiry": [
...
const Chatbot: React.FC = () => {
    ...
    const showOptions = () => {
        ...
        setChatMessages((prev) => [...prev, ...options.map((btn) => btn.props.children as string)]);
    };
    ...
    const handleConversationFlow = (option: string) => {
        ...
        if (conversation && currentStep < conversation.length) {
            ...
            if (step.question) {
                const question = step.question.replace("NAME", userData.name || "").replace("CONTRACTID", userData.contractId || "");
                setChatMessages((prev) => [...prev, question]);
            } else if (step.response) {
                const response = step.response.replace("NAME", userData.name || "").replace("CONTRACTID", userData.contractId || "");
    ...
    return (
        <div className="relative">
            <div className="parent flex justify-between items-center p-8 h-screen">
                <div>
                    <h1 className="text-5xl font-bold text-cadetblue">Lady Nisy's Chatbot</h1>
            ...
            {isChatOpen && (
                <div className="fixed bottom-16 right-16 w-80 bg-white shadow-lg rounded-lg">
                    <div className="header flex items-center p-4 bg-gray-100 rounded-t-lg">
                        <img src="jecrc_logo.png" alt="avatar" className="w-10 h-10 rounded-full border p-1" />
                        <div className="ml-3">
                    ...
                    <div className="chat-body p-4 h-80 overflow-auto">
                        {chatMessages.map((msg, index) => (
                            <p key={index} className="bg-gray-100 text-gray-600 p-2 rounded-lg my-2">
                                {msg}
                            </p>
                        ))}
                    </div>
